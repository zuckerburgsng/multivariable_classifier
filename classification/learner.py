...
def classify(names, labels, attributes, model_dir, model_name, estimators,
    ...
    """
        ...
        - labels: list
            Labels (i.e. 'benign', 'netsupport', 'sagent', sload) of the data used to build a model from.
        - attributes: np.array
...
def parsing_commands():
    ...
    parser.add_argument('--l', metavar='LABEL', type=str, nargs='+',
                        choices=['benign', 'netsupport', 'sagent', 'sload'],
                        help='labels of the JS directories used to build a model from')
    parser.add_argument('--vd', metavar='DIR-VALIDATE', type=str, nargs='+',
                        help='4 JS dir (1 benign, 1 netsupport, 1 sagent, 1 sload) to select the features with chi2')
    parser.add_argument('--vl', metavar='LABEL', type=str, nargs='+',
                        choices=['benign', 'netsupport', 'sagent', 'sload'],
                        help='labels of the 4 JS dir for the features selection process')
...
def main_learn(js_dirs=arg_obj['d'], js_dirs_validate=arg_obj['vd'], labels_validate=arg_obj['vl'],
    ...
    if js_dirs is None:
        logging.error('Please, indicate at least 4 directories (--d option), at least one benign '
                      ', one netsupport, one sagent, one sload (--l option to pass the corresponding labels).\nGot 0 '
                      'directories with the following labels: %s', labels_d)

    elif len(js_dirs) < 2 or labels_d is None\
            or 'benign' not in labels_d or 'netsupport' not in labels_d or 'sagent' not in labels_d or 'sload' not in labels_d:
        ...
        logging.error('Please, indicate the 4 JS directories (--vd option) '
                      'with corresponding labels (--vl option, 1 benign and 1 netsupport, 1 sagent, 1 sload) '
                      'for the features validation process')

    elif (len(js_dirs_validate) != 4 or labels_validate is None\
            or 'benign' not in labels_validate or 'netsupport' not in labels_validate or 'sagent' not in labels_validate or
            'sload' not in labels_validate):
        logging.error('Please, indicate the 4 JS directories (--vd option) '
                      'with corresponding labels (--vl option, 1 benign and 1 netsupport, 1 sagent, 1 sload) '
