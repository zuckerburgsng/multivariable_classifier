...
def predict_labels_using_threshold(names_length, labels_predicted_proba, threshold):
    ...
    labels_predicted_test = ['unknown' for _ in range(names_length)]
    for i, _ in enumerate(labels_predicted_test):
        index_max = np.argmax(labels_predicted_proba[i])
        label_list = ['benign', 'netsupport', 'sagent', 'sload']
        label = label_list[index_max]
        labels_predicted_test[i] = label
...
def get_score(labels, labels_predicted):
    ...
    if '?' in labels:
        ...
        try:
            cm = confusion_matrix(labels, labels_predicted)
            plt.figure(figsize=(9,9))
            sns.heatmap(cm, annot=True, fmt='0.3f', linewidths=0.5,
                        square=True, cbar=True)
            plt.ylabel('Actual Values')
            plt.xlabel('Predicted Values')
            plt.savefig('/home/vboxuser/multivariable_classifier_jstap/database/confusion_matrix/new_figure.png')
